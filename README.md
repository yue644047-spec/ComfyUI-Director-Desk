...
> **当前版本：V1.1** — Neo4j + MySQL 联合素材库（图算法检索）、素材上传、生成产物不入库、favicon/日志等体验优化。
...
- **数据层**：自动保存（0.8s 防抖）、素材库（`assets\` 只收图片素材，上传/打标签，生成产物不入库）、素材检索
  - 素材存储：MySQL `comfyui_assets`（事实/自增 id/使用信息/匹配流水）+ Neo4j（按 id 建 Asset-Tag-FITS 图，图算法检索）；任一库宕机自动回退本地 JSON
