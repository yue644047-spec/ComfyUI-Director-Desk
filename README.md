...
> **当前版本：V1.3** — 资产库（角色/场景/道具 + 六视图设定图 + 自动连线到相关镜头）、变化帧拆解、关键帧链（Flux Kontext 逐帧承接，锁人物与场景一致性）、一键拆变化帧 / 一键生成关键帧 / 生成全部。
>
> V1.2 — 素材匹配升级为本机 Ollama bge-m3 向量语义匹配（离线）、本地 AI 拆镜、多项目管理、Wan2.2 生视频支持。
>
...
- **资产层**：资产库按角色/场景/道具分类，AI 从脚本自动拆出资产并连线到相关镜头，每个资产可生成六视图设定图作为参考
- **关键帧层**：每镜按拍摄时长拆出变化帧，逐帧生成关键帧（Flux Kontext，第 2 帧起承接上一帧，保持一致），帧连成片段后拼接
